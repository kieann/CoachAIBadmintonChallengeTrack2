...
def train(train_dataloader, valid_dataloader, encoder, decoder, location_criterion, shot_type_criterion, encoder_optimizer, decoder_optimizer, args, device="cpu"):
    ...
    for epoch in range(args['epochs']):
        ...
        for rally, target in tqdm(train_dataloader, desc=f"Epoch: {epoch}"):
        ...
        print(f"{loss=}, {loss_location=}, {loss_type=}")
        if loss < best_loss:
            best_loss = loss
...
def our_predict(given_seq, encoder, decoder, args, sample_num, device="cpu"):
    encode_length = args['encode_length']
    encoder.eval(), decoder.eval()

    all_player_A_area_record = []
    all_player_B_area_record = []
    all_shot_type_logits = []
    ...
    with torch.no_grad():        
        ...
        for sample_index in range(sample_num):
            ...
            player_A_area_record = []
            player_B_area_record = []
            shot_type_record = []

            # rally information
            player = given_seq['given_player'].to(device).long()
            shot_type = given_seq['given_shot'].to(device).long()
            player_A_x = given_seq['given_A_x'].to(device)
            player_A_y = given_seq['given_A_y'].to(device)
            player_B_x = given_seq['given_B_x'].to(device)
            player_B_y  = given_seq['given_B_y'].to(device)

            encoder_player = player[0:2].reshape(1, 2)
            encoder_shot_type = shot_type[:encode_length-1].reshape(1, encode_length-1)
            encoder_player_A_x = player_A_x[:encode_length].reshape(1, encode_length)
            encoder_player_A_y = player_A_y[:encode_length].reshape(1, encode_length)
            encoder_player_B_x = player_B_x[:encode_length].reshape(1, encode_length)
            encoder_player_B_y = player_B_y[:encode_length].reshape(1, encode_length)
            ...
            decoder_player = player[0:2].reshape(1, 2)

            decoder_player_A_x = player_A_x[encode_length-1:encode_length].reshape(1, 1)
            decoder_player_A_y = player_A_y[encode_length-1:encode_length].reshape(1, 1)
            decoder_player_B_x = player_B_x[encode_length-1:encode_length].reshape(1, 1)
            decoder_player_B_y = player_B_y[encode_length-1:encode_length].reshape(1, 1)
            
            first = True
            for sequence_index in range(encode_length, min(given_seq['rally_length']+1, args['max_length']+1)): # +1 since landing area is next player's location
                predict_xy, predict_shot_type_logit, adjacency_matrix, decoder_node_embedding, original_embedding = decoder(decoder_player, sequence_index+1, decoder_node_embedding, original_embedding, adjacency_matrix, 
                                                                                                                        decoder_player_A_x, decoder_player_A_y, decoder_player_B_x, decoder_player_B_y,
                                                                                                                        shot_type=None, train=False, first=first)
                ...
                weights = F.softmax(predict_shot_type_logit, dim=-1)
                shot_type_record.append(weights[0, 1:].cpu().tolist())

                player_A_area_record.append((predict_A_xy[0, 0, 0].item(), predict_A_xy[0, 0, 1].item()))
                player_B_area_record.append((predict_B_xy[0, 0, 0].item(), predict_B_xy[0, 0, 1].item()))

            all_player_A_area_record.append(player_A_area_record)
            all_player_B_area_record.append(player_B_area_record)
            all_shot_type_logits.append(shot_type_record)

    return all_player_A_area_record, all_player_B_area_record, all_shot_type_logits
